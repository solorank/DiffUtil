...
public class DiffAdapter extends RecyclerView.Adapter<DiffAdapter.DiffVH> {
    ...
    public DiffAdapter(Context mContext, List<JavaBean> mDatas) {
        this.mContext = mContext;
        this.mDatas = mDatas;
        mInflater = LayoutInflater.from(mContext);
    }

    public void setDatas(List<JavaBean> mDatas) {
        this.mDatas = mDatas;
    }
    ...
    public void onBindViewHolder(final DiffVH holder, final int position) {
        JavaBean bean = mDatas.get(position);
    ...
    public void onBindViewHolder(DiffVH holder, int position, List<Object> payloads) {
        if (payloads.isEmpty()) {
            ...
            JavaBean bean = mDatas.get(position);//取出新数据源，（可以不用）
